...
export class MessageProcessor {
  ...
  private async handleSend(from: string, args: string[]) {
    ...
    if (!recipient || !recipient.stellarWallet) {
      return await this.whatsappService.sendMessage(from, t('send.no_recipient', lang, { target }));
    }

    const senderWallet = JSON.parse(sender.stellarWallet);
    const senderSecret = decrypt(
      senderWallet.encryptedSecret,
      senderWallet.iv,
    ...
    try {
      await this.whatsappService.sendMessage(from, t('send.initiating', lang, { amount, target }));
      await this.stellarService.sendPayment(senderSecret, recipientPublicKey, amount);
      await this.whatsappService.sendMessage(from, t('send.success', lang, { amount, target }));
    } catch (e: any) {
      console.error(e);
      await this.whatsappService.sendMessage(
        from,
  ...
  private async handleRequest(from: string, args: string[]) {
    ...
    const recipient = await this.userService.resolveUser(target);
    if (!recipient) {
      return await this.whatsappService.sendMessage(from, t('request.no_user', lang, { target }));
    }
  ...
  private async handleInviteMember(from: string, args: string[]) {
    ...
    if (!adminGroup) {
      return await this.whatsappService.sendMessage(from, t('invite_member.not_creator', lang));
    }
    ...
    await this.whatsappService.sendMessage(from, t('invite_member.success', lang, { target }));
  }
  ...
  private async handleGroupStatus(from: string, _args: string[]) {
    ...
    if (memberships.length === 0) {
      return await this.whatsappService.sendMessage(from, t('group_status.no_groups', lang));
    }
